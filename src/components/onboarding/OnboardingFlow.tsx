...
import { LEVELS, INTERESTS, GOALS, labelFor, uiLanguageFor, type LearnDirection } from "@/lib/onboarding";
            ...
            <Step title={t("onbLevelTitle")} body={t("onbLevelBody")}>
              <div className="space-y-2.5">
                {LEVELS.map((lv) => (
                  ...
                  >
                    <span className="rounded-full bg-sage/60 px-2 py-0.5 text-[11px] font-semibold tracking-wide text-charcoal/70">
                      {lv.id}
                    </span>
                    <span className="min-w-0">
                      <span className="block text-sm font-medium text-charcoal">{lang === "ko" ? lv.ko : lv.en}</span>
...
function ChoiceCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center justify-between gap-3 rounded-xl2 border p-5 text-left transition ${
